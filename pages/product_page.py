...
class ProductPage(BasePage):
    ...
    def check_product_name(self):
        ...
        assert self.is_element_present(
            *ProductPageLocators.BASKET_CONFIRM
        ), "Имя в корзине не найдено"
        product_name = self.browser.find_element(*ProductPageLocators.PRODUCT_NAME).text
        product_confirm = self.browser.find_element(
            *ProductPageLocators.BASKET_CONFIRM
        ).text
        assert (
            product_name == product_confirm
        ), "Название продукта в корзине не совпадает"
